    ...
    void extract(const BSONObj& doc) {
        ...
        for (BSONObjIterator it(doc); it.more(); ) {
            ...
            if (slotIt != _sigToSlot.end()) {
                ...
                // 验证字段名匹配（防止签名冲突）
                if (_fields[slot].size() == fieldNameLen &&
                ...
                if (prefixIt != _nestedPrefixSigs.end()) {
                    ...
                    for (uint8_t slot : prefixIt->second) {
                        if (_slots[slot].eoo()) {
                            ...
                            _slots[slot] = dotted_path_support::extractElementAtPath(subObj, subPath);
                            if (!_slots[slot].eoo()) {
                                ++_extractedCount;
                            }
                        }
...
private:
    // ===== 注册信息 =====
    std::unordered_map<uint32_t, uint8_t> _sigToSlot;   // 签名 → 槽位
