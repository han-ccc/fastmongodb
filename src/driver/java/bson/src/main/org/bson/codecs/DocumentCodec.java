...
import org.bson.IntegrityVerifiedDocument;
...
public class DocumentCodec implements CollectibleCodec<Document> {
    ...
    private void writeMap(final BsonWriter writer, final Map<String, Object> map, final EncoderContext encoderContext) {
        ...
        // If this is an IntegrityVerifiedDocument that is locked, write the hash field first
        if (map instanceof IntegrityVerifiedDocument) {
            IntegrityVerifiedDocument ivd = (IntegrityVerifiedDocument) map;
            if (ivd.isLocked() && ivd.getIntegrityHash() != null) {
                writer.writeName(IntegrityVerifiedDocument.DOC_HASH_FIELD_NAME);
                writer.writeInt64(ivd.getIntegrityHash());
            }
        }
        ...
        for (final Map.Entry<String, Object> entry : map.entrySet()) {
            ...
            // Skip the hash field if it was already written
            if (IntegrityVerifiedDocument.DOC_HASH_FIELD_NAME.equals(entry.getKey())) {
                continue;
            }
